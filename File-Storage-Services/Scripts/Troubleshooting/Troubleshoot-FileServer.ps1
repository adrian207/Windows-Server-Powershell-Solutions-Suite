...
<#
.SYNOPSIS
    File Server Troubleshooting and Diagnostic Script
...
.PARAMETER Action
    The action to perform (Diagnose, Repair, Monitor, Analyze, Report, TestShare)
...
.PARAMETER ShareName
    Name of share to test (for TestShare action)

.PARAMETER TestUser
    User account to test share access with

.PARAMETER GenerateReport
    Generate detailed HTML report

.EXAMPLE
    .\Troubleshoot-FileServer.ps1 -Action Diagnose

.EXAMPLE
    .\Troubleshoot-FileServer.ps1 -Action Repair -RepairType "Services"

.EXAMPLE
    .\Troubleshoot-FileServer.ps1 -Action Monitor -MonitorDuration 600 -MonitorInterval 15

.EXAMPLE
    .\Troubleshoot-FileServer.ps1 -Action Report -OutputPath "C:\Reports\FileServer-Report.html" -GenerateReport

.EXAMPLE
    .\Troubleshoot-FileServer.ps1 -Action TestShare -ShareName "Data" -TestUser "Domain\User"
...
param(
    ...
    [ValidateSet("Diagnose", "Repair", "Monitor", "Analyze", "Report", "TestShare")]
    [string]$Action,
...
function Start-FileServerDiagnosis {
    Write-TroubleshootingLog "Starting comprehensive file server diagnosis..." "INFO"
    ...
    try {
        ...
        # Check storage
        Write-TroubleshootingLog "Checking storage..." "INFO"
        $storageHealth = Test-StorageHealth
        $diagnosisResults.Storage = $storageHealth
        ...
        if ($IncludeLogs) {
            ...
            $logEntries = Get-FileServerLogEntries -StartTime $startTime
        ...
        if ($diagnosisResults.Configuration.FileServerInstalled -ne $true) {
            $diagnosisResults.Issues += "File server role not installed"
            $diagnosisResults.Recommendations += "Install File Server role"
        }
        ...
        if ($diagnosisResults.Storage.Issues.Count -gt 0) {
            $diagnosisResults.Issues += $diagnosisResults.Storage.Issues
            $diagnosisResults.Recommendations += $diagnosisResults.Storage.Recommendations
        }
...
function Start-FileServerRepair {
    param([string]$RepairType)
    
    Write-TroubleshootingLog "Starting file server repair operation..." "INFO"
    
    try {
        $repairResults = Repair-FileServerInstallation -RepairType $RepairType
...
function Start-FileServerMonitoring {
    param(
        [int]$Duration,
        [int]$Interval
    )
    
    Write-TroubleshootingLog "Starting file server monitoring for $Duration seconds..." "INFO"
    
    try {
        Watch-FileServerPerformance -Duration $Duration -Interval $Interval
...
function Start-FileServerAnalysis {
    param([int]$LogDays)
    
    Write-TroubleshootingLog "Starting file server log analysis..." "INFO"
...
function New-FileServerReport {
    ...
    Write-TroubleshootingLog "Generating file server report..." "INFO"
    
    try {
        if (-not $OutputPath) {
            $OutputPath = Join-Path $scriptPath "FileServer-Report-$(Get-Date -Format 'yyyyMMdd-HHmmss').html"
        }
        
        $report = @{
            Title = "File Server Diagnostic Report"
            Generated = Get-Date
            ComputerName = $env:COMPUTERNAME
            Domain = $env:USERDOMAIN
        ...
        $htmlReport = $report | ConvertTo-Html -Title "File Server Diagnostic Report" -Head @"
<style>
...
try {
    Write-TroubleshootingLog "Starting file server troubleshooting..." "INFO"
    ...
    switch ($Action) {
        "Diagnose" {
            $results.Diagnosis = Start-FileServerDiagnosis
            
            if ($results.Diagnosis) {
                Write-Host "`n=== File Server Diagnosis Results ===" -ForegroundColor Cyan
        ...
        "Repair" {
            $results.Repair = Start-FileServerRepair -RepairType $RepairType
            
            if ($results.Repair) {
                Write-Host "`n=== File Server Repair Results ===" -ForegroundColor Cyan
                ...
                Write-Host "Storage Repaired: $($results.Repair.StorageRepaired)" -ForegroundColor White
                Write-Host "Shares Repaired: $($results.Repair.SharesRepaired)" -ForegroundColor White
            }
        }
        
        "Monitor" {
            $results.Monitoring = Start-FileServerMonitoring -Duration $MonitorDuration -Interval $MonitorInterval
        }
        
        "Analyze" {
            $results.Analysis = Start-FileServerAnalysis -LogDays $LogDays
            
            if ($results.Analysis) {
                Write-Host "`n=== File Server Analysis Results ===" -ForegroundColor Cyan
        ...
        "Report" {
            $diagnosisResults = Start-FileServerDiagnosis
            $analysisResults = Start-FileServerAnalysis -LogDays $LogDays
            
            $reportPath = New-FileServerReport -DiagnosisResults $diagnosisResults -AnalysisResults $analysisResults -OutputPath $OutputPath
            
            if ($reportPath) {
                Write-Host "`n=== File Server Report Generated ===" -ForegroundColor Cyan
        ...
        "TestShare" {
            if (-not $ShareName) {
                throw "ShareName parameter is required for TestShare action"
            }
            
            $results.ShareTest = Test-FileShareAccess -ShareName $ShareName -TestUser $TestUser
            
            if ($results.ShareTest) {
                Write-Host "`n=== File Share Test Results ===" -ForegroundColor Cyan
                Write-Host "Share Name: $($results.ShareTest.ShareName)" -ForegroundColor White
                Write-Host "Share Exists: $($results.ShareTest.ShareExists)" -ForegroundColor White
                Write-Host "Share Accessible: $($results.ShareTest.ShareAccessible)" -ForegroundColor White
                Write-Host "Issues Found: $($results.ShareTest.Issues.Count)" -ForegroundColor White
                
                if ($results.ShareTest.Issues.Count -gt 0) {
                    Write-Host "`nIssues:" -ForegroundColor Yellow
                    $results.ShareTest.Issues | ForEach-Object { Write-Host "  - $_" -ForegroundColor Red }
                }
                
                if ($results.ShareTest.Recommendations.Count -gt 0) {
                    Write-Host "`nRecommendations:" -ForegroundColor Yellow
                    $results.ShareTest.Recommendations | ForEach-Object { Write-Host "  - $_" -ForegroundColor Green }
                }
            }
        }
    }
    ...
    Write-TroubleshootingLog "File server troubleshooting completed successfully in $($duration.TotalMinutes.ToString('F2')) minutes." "SUCCESS"
    
    # Display final status
    Write-Host "`n=== File Server Troubleshooting Summary ===" -ForegroundColor Cyan
    ...
    Write-TroubleshootingLog "File server troubleshooting failed after $($duration.TotalMinutes.ToString('F2')) minutes: $($_.Exception.Message)" "ERROR"
